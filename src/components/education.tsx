...
export function Education({ educations }: EducationProps) {
	return (
		<div className={cn('font-light text-zinc-600 tracking-widest print:tracking-wider')}>
			...
			<div className='flex flex-col mt-6 space-y-4 print:space-y-4'>
				{educations?.map((edu) => (
					<div className='flex flex-col space-y-1 print:space-y-0 print:pr-4' key={edu.id}>
						<h4 className=' uppercase  text-zinc-600 text-sm font-semibold print:text-[10px] print:font-medium print:leading-snug'>
							{edu?.field}
						</h4>
						...
						<div className='flex items-center justify-start gap-2'>
							...
							<small className='text-gray-400 print:hidden text-xs'>
								{calculateDuration(edu.startDate.toString(), edu?.endDate?.toString())}
							</small>
						</div>
					</div>
				))}
